...
const arrayPopulationNumber = arrayPopulation.map((item)=>item.textContent).map((str)=>str.split(",")).map((arrStr)=>arrStr.map((el)=>Number.isFinite(+el) ? el : "0")).map((arr)=>+arr.join(""));
